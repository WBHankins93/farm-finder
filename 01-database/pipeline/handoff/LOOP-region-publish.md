...
flowchart TD
    ...
    PICK -->|"pending (has config)"| COLLECT
    ...
    SDONE --> LEDGER[update ledger] --> NEXT([schedule next ~60s])
    SREV --> LEDGER

    PICK -->|"all collectable done"| PUB
    subgraph ROLLUP["region rollup (gate)"]
      PUB["run.py --publish<br/>aggregate ALL states"] --> FEED["build/app-farms.json<br/>(national snapshot)"]
      FEED --> LOAD["load_postgres.py --refresh<br/>TRUNCATE + COPY (idempotent)"]
      LOAD --> PG[("Postgres<br/>farms table")]
    end
    PG --> MARK["mark region published"] --> NEXT
```

**One firing = one path through the graph:** either it collects a single state
(the `UNIT` box) *or* it finalizes a region (the `ROLLUP` box) — never both, never
two states. Progress is read fresh from disk each firing, so a crash just re-runs
the current unit harmlessly.
...
2. **Find the frontier.** Pick the **first region not yet marked `published`** in
   ...
   - **blocked** — no `sources/*/<ST>.json` config exists. Cannot collect (e.g.
     KY, VA, WV and every state without an authored config). Record `blocked`
     with reason "no source config"; skip.
...
3. **Collect one pending state** (one per iteration):
   ...
   - Ledger: `states.<ST> = {collected_at, eligible, residue, residue_nongeo,
     ...
     count is surfaced for a human later). **End the iteration here** and schedule
     the next wake.
...
4. **Publish + load when the region's collectable states are all done.** If every
   ...
   - Verify: `psql -d farmfinder -tA -c "SELECT count(*) FROM farms;"` — record the
     row count.
   - Ledger: `regions.<region> = {published:true, published_at, db_count,
     blocked_states:[...], review_states:[...]}`. **End the iteration.**
...
- Don't commit or push. Working-tree changes only.
