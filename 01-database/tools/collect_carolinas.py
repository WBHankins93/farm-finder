...
def finalize(state: str, observations: list[dict[str, Any]], raw_records: list[dict[str, Any]], logs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for index, (name, url, count, digest, note) in enumerate(source_plan(state, logs), 1):
        source_pass = 1 if index <= pass_one_end else 2 if index <= pass_two_end else 3
        source_list.append({"sourceId": f"{state.lower()}src-{index:02d}", "name": name, "pass": source_pass, "sourceUrl": url, "repositoryPath": None, "decision": "observations_retained" if count else "queued_for_collection", "recordsParsed": count, "retrievedAt": RETRIEVED_AT, "responseSha256": digest, "notes": note})
...
def source_plan(state: str, logs: list[dict[str, Any]]) -> list[tuple[str, str, int, str | None, str]]:
    ...
    if state == "NC":
        ...
        return [("NCDA&CS — N.C. Farmer/Agribusiness Listing (June 22, 2026)", NC_XLSX_URL, primary.get("records_parsed", 0), primary.get("sha256"), "Official workbook; every named row retained."), ("U.S. Census Bureau — North Carolina county denominator", NC_COUNTIES_URL, 100, None, "Denominator loaded."), ("Visit NC Farms — Our Farms and Businesses", VISIT_NC_FARMS_URL, visit.get("records_parsed", 0), visit.get("sha256"), "Public farm-directory pool collected; county and identity corroboration remains QA."), ("Got to Be NC — growers and producers", GOTTOBENC_URL, gottobenc.get("records_parsed", 0), gottobenc.get("sha256"), "Complete 59-page growers/producers directory pool collected; profile-level contact and county review remains QA."), ("U.S. Farm Trail — North Carolina", US_FARM_TRAIL_API + "?state=north-carolina", trail.get("records_parsed", 0), trail.get("sha256"), "Complete public GeoJSON directory pool collected; county and identity corroboration remains QA."), ("EatWild — North Carolina", EATWILD_URLS["NC"], eatwild.get("records_parsed", 0), eatwild.get("sha256"), "Pastured-products directory collected; county and current-operation review remains QA."), ("PickYourOwn — North Carolina", PICKYOUROWN_BASE + "NC.htm", sum(row.get("records_parsed", 0) for row in pyo), None, "Regional U-pick pages collected; current-operation review remains QA."), ("LocalHarvest — North Carolina", "https://www.localharvest.org/north-carolina/", 0, None, "Queued for a follow-up source pass.")]
    ...
    return [("Certified SC Grown — REST member directory", SC_API_URL, sum(row.get("records_parsed", 0) for row in api_logs), None, "Complete certified-sc-grown REST member pool; profile pages provide field enrichment."), ("Certified SC Grown — member profiles", SC_PROFILE_BASE, sum(row.get("records_parsed", 0) for row in logs if row.get("source_name") == "Certified SC Grown — member profile"), None, "Complete profile enrichment pass."), ("U.S. Census Bureau — South Carolina county denominator", SC_COUNTIES_URL, 46, None, "Denominator loaded."), ("South Carolina Department of Agriculture — agritourism", SC_AGRITOURISM_URL, agritourism.get("records_parsed", 0), agritourism.get("sha256"), "State agritourism page and participating-farms PDF collected."), ("U.S. Farm Trail — South Carolina", US_FARM_TRAIL_API + "?state=south-carolina", trail.get("records_parsed", 0), trail.get("sha256"), "Complete public GeoJSON directory pool collected; county and identity corroboration remains QA."), ("EatWild — South Carolina", EATWILD_URLS["SC"], eatwild.get("records_parsed", 0), eatwild.get("sha256"), "Pastured-products directory collected; county and current-operation review remains QA."), ("PickYourOwn — South Carolina", PICKYOUROWN_BASE + "SC.htm", sum(row.get("records_parsed", 0) for row in pyo), None, "Regional U-pick pages collected; current-operation review remains QA."), ("LocalHarvest — South Carolina", "https://www.localharvest.org/south-carolina/", 0, None, "Queued for a follow-up source pass.")]
